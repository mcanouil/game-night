---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->

# Game Night Poster

<!-- badges: start -->
[![License](https://img.shields.io/github/license/mcanouil/game-night)](LICENSE)
<!-- badges: end -->

```{r, echo = FALSE, message = FALSE}
source("assets/social.R")
library(magick)

rscripts <- list.files(path = "R", pattern = "^20.*\\.R$", full.names = TRUE)
posters <- sub("\\.R$", "", basename(rscripts))
pngs <- sprintf("posters/%s/%s.png", posters, posters)
thumbs <- sprintf("thumbs/%s.png", posters)

outdated <- !file.exists(thumbs) |
  min(sapply(c(pngs, thumbs), file.mtime)) < max(
    sapply(
      X = c(rscripts, "assets/social.R"),
      FUN = file.mtime
    )
  )

invisible(lapply(rscripts[outdated], source, encoding = "UTF-8"))
invisible(Map(
  f = function(path_in, path_out) {
    image <- magick::image_read(path_in)
    image <- magick::image_resize(image, "192x")
    magick::image_write(image, path_out)
  }, 
  sub("\\.png", "_01.png", pngs)[outdated],
  thumbs[outdated]
))
unlink(dirname(pngs)[as.Date(posters) < Sys.Date()], recursive = TRUE)
```

```{r, results = "asis", echo = FALSE}
cell <- sprintf(
  paste0(
    '<td align = "center">',
      '<a href="%s>',
        '<img alt="Poster for %s game night" src="%s" width="192" height="108" />',
        "<br/>",
        "%s",
      "</a>",
    "</td>"
  ),
  dirname(pngs), posters, thumbs, posters
)

cols <- 4
rows <- ceiling(length(cell) / cols)

row_id <- rep(seq_len(rows), each = cols, length.out = length(cell))
row_cells <- split(cell, row_id)

cat("<table>\n")
cat(paste0("<tr>", sapply(row_cells, paste, collapse = ""), "</tr>"), sep = "")
cat("</table>\n")
```
